 ...
 * library of libkenlm (libkenlm.dll, libkenlm.so, etc.) according to the user
 * platform (<i>os.name</i> and <i>os.arch</i>). The natively compiled libraries
 * bundled to libkenlm contain the codes of the original libkenlm and JNI
 * programs to access libkenlm.
 * <p>
 ...
 * <p>
 * This KenLMLoader searches for native libraries (libkenlm.dll, libkenlm.so,
 * etc.) in the following order:
 * <ol>
 * <li>If system property <i>com.github.jbaiter.kenlm.use.systemlib</i> is set
 * to true, lookup folders specified by <i>java.lib.path</i> system property
 * (This is the default path that JVM searches for native libraries)
 * <li>(System property: <i>com.github.jbaiter.kenlm.lib.path</i>)/(System
 * property: <i>com.github.jbaiter.kenlm.lib.name</i>)
 ...
 * <i>/tmp/vinhkhuc</i> as a temporary folder to copy native libraries, use -D
 * option of JVM:
 * <p>
 * 
 * <pre>
...
public class KenLMLoader {
	...
	private static void loadProperties() {
		try {
			...
			System.err
					.println("Could not load '" + KENLM_SYSTEM_PROPERTIES_FILE + "' from classpath: " + ex.toString());
		}
	}
	 ...
	 * {@link System#load(String)} method uses the class loader of the caller class
	 * when loading native libraries.
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * <p>
	 * Note that Java's class loader first delegates the class lookup to its parent
	 * class loader. So once CrfSuiteNativeLoader is loaded by the root class
	 * loader, no child class loader initialize CrfSuiteNativeLoader again.
	 *
	 * @throws Exception
	 */
	...
	/**
	 * Load kenlm's native code using load method of the KenMLNativeLoader class
	 * injected to the root class loader.
	 *
	 * @throws Exception
	 */
...
//		if (libraryName == null || "".equals(libraryName)) {
		System.load(KenLMLoader.class.getResource("kenlm-jni.so").getPath());
		System.out.println("path loading : " + KenLMLoader.class.getResource("kenlm-jni.so").getPath());
//		} else {
//			try {
...
//        }
	...
	static File findNativeLibrary() throws Exception {
		...
		boolean disabledBundledLibs = Boolean.parseBoolean(System.getProperty(KEY_KENLM_DISABLE_BUNDLED_LIBS, "false"));
...
//		System.out.println("path loading : " + KenLMLoader.class.getResource("kenlm-jni.so").getPath());
//		System.out.println("Kenlm path 2 : " + KenLMLoader.class.getResource("kenlm-jni.so").getPath());
	}
}
